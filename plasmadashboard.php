...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-fluid border"><h3>
            <?php echo $row['name']; ?> 's Dashboard with ID: <?php echo $pid; ?></h3>
        </div>
        ...
        <?php
                ...
                if(isset($_POST['query'])){
                    $s = $_POST['query'];
                        // echo $s;
                    $donors = "SELECT * FROM users WHERE type = 'donor' AND pid = '$pid' AND name LIKE '%$s%' OR email LIKE '%$s%' OR ";
                    $result = mysqli_query($conn,$donors);
                } else {
                    $donors = "SELECT * FROM users WHERE type = 'donor' AND pid = '$pid' ";
                    $result = mysqli_query($conn,$donors);
                }
        ?>
            ...
            <form method="POST" actions="plasmadashboard.php">
            <div class="form-group px-5">
            <label for="usr"></label>
            <input type="text" class="form-control" name="query" placeholder="Search Donors">
            ...
            <table class="table">
                <thead class="thead-inverse">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><a href="donorupdate.php?donorid=<?php echo $row['id'] ?>" class="btn btn-info" role="button">Update</a></td>
